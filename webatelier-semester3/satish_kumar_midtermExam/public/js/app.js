...
function InitThis() {
    if(localStorage.getItem('color')){
      colorpen = localStorage.getItem('color');
    ...
    $('#myCanvas').mousemove(function (e) {
        if (mousePressed) {
            ...
            socket.emit('drawer',{'x':e.pageX - $(this).offset().left,'y':e.pageY - $(this).offset().top,'isDown':true})
        }
    });
...
function save(){
  ...
  localStorage.setItem('color',colorpen )
...
function makePrivate(){
  console.log(document.getElementById('inpu').value)
  var datas = {

    
    'firstName': null,
    'lastName':null,
    'userName':document.getElementById('inpu').value,
    'email' : null,

  }
  doJSONRequest("POST", "/users", null, datas, redirect);
  function redirect(){};
}
...
function showsssArtist(){
  doJSONRequest("GET", "/artists", null, null, renderArtist);
    ...
    function renderArtist(artists){
        ...
        // function renderShowArtist(artists){

        var artistData = [];
     for(album in artists){
      if (m == 0){
          if (artists[album].like == "yes"){
             var newAlbumData = {};
          

              newAlbumData.artwork = artists[album].artwork;
              newAlbumData._id = artists[album]._id;
              newAlbumData.name = artists[album].name;
              newAlbumData.genre = artists[album].genre;
              newAlbumData.like = artists[album].like;
              artistData.push(newAlbumData);
