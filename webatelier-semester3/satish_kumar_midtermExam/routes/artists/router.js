...
router.put('/:artistid', function(req, res, next) {
  ...
  Artist.findById(req.params.artistid, fieldsFilter, function(err, artist){
    ...
    if (artist){
      ...
      artist.genre = data.genre?data.genre:artist.genre;
      artist.artwork = data.artwork?data.artwork:artist.artwork;
      artist.dateCreated = data.dateCreated?data.dateCreated:artist.dateCreated;
      artist.like = data.like?data.like:artist.like;
