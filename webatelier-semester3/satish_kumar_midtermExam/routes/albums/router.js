...
router.put('/:albumid', function(req, res, next) {
  ...
  Album.findById(req.params.albumid, fieldsFilter , function(err, album){
    ...
    if (album){
      ...
      album.artist = data.artist?data.artist:album.artist;
      album.artwork = data.artwork?data.artwork:album.artwork;
      album.dateCreated = data.dateCreated?data.dateCreated:album.dateCreated;
    
      album.like=data.like?data.like:album.like;
