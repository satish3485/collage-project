...
router.get('/', function (req, res, next) {
    ...
    Track.find({}, fieldsFilter).lean().populate('artist').populate('album').exec(function (err, tracks) {
...
router.get('/:trackid/edit', function (req, res, next) {
    Track.findById(req.params.trackid, fieldsFilter, function (err, track) {
        ...
        trackToReturn.push({
            field_name: "name",
            field_value: track.name
        });
        trackToReturn.push({
            field_name: "dateCreated",
            field_value: track.dateCreated
        });
        trackToReturn.push({
            field_name: "id3Tags",
            field_value: JSON.stringify(track.id3Tags).replace("]", "").replace("[", "").replace(/"/g, "")
        });
        trackToReturn.push({
            field_name: "dateReleased",
            field_value: track.dateReleased
        });
        trackToReturn.push({
            field_name: "file",
            field_value: track.file
        });
        trackToReturn.push({
            field_name: "duration",
            field_value: track.duration
        });
        trackToReturn.push({
            field_name: "album",
            field_value: track.album
        });
        trackToReturn.push({
            field_name: "artist",
            field_value: track.artist
        });
